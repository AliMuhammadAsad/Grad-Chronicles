...
# The provided script is a Bash script for mounting and installing USB pen/keychain storage devices on a Linux System
# It is designed to run as the root user and is typically triggered by the hotplug system when a USB storage device is plugged in or removed.
# The hotplug system that allows the system to automatically detect and manage hardware devices including USBs when they are added or removed on a running system.
# It is designed to run as the root user as mounting and unmounting typically requires root privileges because these operations involve low-level interactions with the hardware and file system. Moreover, the script may need to modify system-wide configuration files or directories. USB devices can potentially introduce security risks, such as malicious code or unauthorized access to the system. Running the script as root allows it to make security-related decisions and apply access controls to mitigate potential risks.
...
# -------------------------------------------------------------------------------------------------------------- #
# These lines provide the installation instructions as to the specific directory in which it must be in, and the configuration files that must be modified to ensure that the script is run when a USB device is plugged in or removed.
...
if [ "${ACTION}" = "add" ] && [ -f "${DEVICE}" ]; then 
    ...
    for procEntry in $(allAttachedScsiUsb); do
        ...
        if ! grep -q $prodid $DEVLABELCONFIG; then #if this is false, -q stands for quiet and tells grep to operate silently (i.e. do not display any output)
            ...
            $DEVLABEL add -d $part -s $SYMLINKDEV >/dev/null #add the device label using 'add' to the device through -d from the source using option -s
        fi
        #
